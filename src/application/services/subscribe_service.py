...
class SubscribeService:
    ...
    async def check_subscribes_user(self,user_id:int,type_id:int,session):
        ...
        """
        return self.check_subscribes_user_use_case.execute(user_id=user_id,type_id=type_id,session=session)

    async def subscribe(self,user_id:int,type_id:int,session):
        return self.subscribe_user_use_case.execute(user_id=user_id,type_id=type_id,session=session)

    async def unsubscribe(self,user_id:int,type_id:int,session):
        return self.unsubscribe_user_use_case.execute(user_id=user_id,type_id=type_id,session=session)
